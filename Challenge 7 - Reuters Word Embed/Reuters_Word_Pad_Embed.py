...
one_hot_train_labels = np_utils.to_categorical(y_train,46)
one_hot_test_labels = np_utils.to_categorical(y_test,46)
...
NumEpochs = 30
BatchSize = 32
#early_stop =tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0, patience=0, verbose=1, mode='auto', baseline=None, restore_best_weights=False)
history = model.fit(train_padded,one_hot_train_labels, epochs=NumEpochs, batch_size=BatchSize, validation_data=(test_padded, one_hot_test_labels))
